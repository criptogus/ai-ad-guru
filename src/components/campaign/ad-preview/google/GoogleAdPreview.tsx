    ...
    <div className="relative">
      <div className={`bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md p-3 font-sans shadow-sm transition-all ${viewMode === "mobile" ? "max-w-sm mx-auto" : "max-w-2xl"}`}>
        ...
        <div className="text-blue-800 dark:text-blue-400 text-xl font-medium leading-tight cursor-pointer">
          {ad.headlines.map((headline, i) => (
            <React.Fragment key={i}>
              ...
              <span className="hover:underline">{headline}</span>
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="flex justify-end mt-2">
          <svg xmlns="http://www.w3.org/2000/svg" width="40" height="16" viewBox="0 0 272 92" className="opacity-50">
      ...
      {showEditControls && (
        <div className="absolute top-2 right-2 bg-white/95 dark:bg-gray-800/95 border border-gray-200 dark:border-gray-700 shadow-sm rounded p-2 text-xs">
          <div className="flex items-center justify-between mb-1">
            <span>Headlines:</span>
            <span className={`${
              ad.headlines.reduce((acc, h) => acc + h.length, 0) > 85 ? 'text-red-500' : 'text-gray-500'
            }`}>
              {ad.headlines.reduce((acc, h) => acc + h.length, 0)}/90 chars
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span>Descriptions:</span>
            <span className={`${
              ad.descriptions.reduce((acc, d) => acc + d.length, 0) > 170 ? 'text-red-500' : 'text-gray-500'
            }`}>
              {ad.descriptions.reduce((acc, d) => acc + d.length, 0)}/180 chars
            </span>
          </div>
        </div>
      )}
